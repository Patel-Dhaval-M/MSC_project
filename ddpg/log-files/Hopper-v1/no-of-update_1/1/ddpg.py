...
def train(sess, env, args, actor, critic, actor_noise, logger):
...
#        for j in range(env.spec.max_episode_steps or int(args['max_episode_len'])):
        ...
        #for j in range(int(args['max_episode_len'])):
            ...
            if terminal:
                ...
                if i % 20 == 0:
                    mean_reward = run_policy(env, actor, logger, 10)
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        train(sess, env, args, actor, critic, actor_noise, logger)
        logger.close()
        if args['use_gym_monitor']:
            env.monitor.close()
...
#    parser.add_argument('--monitor-dir', help='directory for storing gym results', default=os.path.join('results/videos') , args['env'], now)#'./results/gym_ddpg')
#    parser.add_argument('--summary-dir', help='directory for storing tensorboard info', default=os.path.join('results/logs', args['env'], now))#'./results/tf_ddpg')
